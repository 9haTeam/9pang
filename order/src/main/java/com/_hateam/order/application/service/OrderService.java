...
public class OrderService {
    ...
    @Transactional
    public OrderResponseDto sequenceCreateOrder(OrderRequestDto requestDto) {
        Order order = createBasicOrder(requestDto);

        // 각 주문 상품에 대해 재고 확인 및 감소 요청을 Kafka로 전달
        requestDto.getProducts().forEach(productDto -> {
            // StockCheckCommand: 주문ID, 상품ID, 요청 수량 정보를 포함하는 도메인 모델
            StockCheckCommand stockCheckCommand = new StockCheckCommand(
                    order.getId(),
                    productDto.getProductId(),
                    productDto.getQuantity()
            );
            // CHECK_STOCK 토픽으로 메시지 전송
            kafkaTemplate.send(OrderStatus.CHECK_STOCK.getOrderStatus(), order.getId(), stockCheckCommand);
        });

    }

    private Order createBasicOrder(OrderRequestDto requestDto) {
